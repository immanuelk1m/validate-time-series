...
이 문서는 두 스킬이 공유하는 실행 계약이다. 계획은 plan-forecast, 생성된 결과의 감사·채점은 validation-forecast가 담당한다. 모델 학습·예측은 별도 실행 단계다.

`$SKILL_DIR`는 공통 실행기를 보관한 validation-forecast 디렉터리다. `$PLAN_SKILL_DIR`는 같은 checkout의 plan-forecast 디렉터리다. 두 스킬을 함께 설치한다.
...
## 2. plan-forecast에서 평가 계획 동결

```bash
.venv/bin/python "$PLAN_SKILL_DIR/scripts/plan_forecast.py" \
  --protocol ./protocol.json --data ./target.csv --out ./runs/v1
```
...
`fold_plan.csv`와 `planning_summary.json`에 시계열별 rolling Fold 수·기간·학습 관측 수와 별도 보고 블록 수를 기록한다. `--expected-folds K`를 쓰면 실제 계획 수가 K와 다를 때 중단한다. 계획을 고정해도 모델은 실행되지 않는다. `runs/v1/plan/` 아래 기존 계획에는 모델용 일정 `origins.jsonl`, 평가 전용 정답 `expected.jsonl`, 공통 경계 제외 기록과 `lock.json`이 생성된다. **expected 원장을 adapter에 전달하지 않는다.** scorer는 별도 프로세스에서 실행하는 것이 원칙이다.

## 3. 별도 모델 실행 단계에서 기준선 생성
...
아래 baseline 명령은 사용자가 예측 실행을 요청한 경우에만 모델 실행자가 사용한다. 계획 전용 또는 사후 검증 요청에 자동으로 추가하지 않는다.
...
## 5. validation-forecast에서 채점

```bash
.venv/bin/python "$SKILL_DIR/scripts/validation_forecast.py" \
  --plan ./runs/v1/plan \
...
[준수표 틀](../assets/compliance-template.csv)은 agent/검토자가 실제 Fold·fit·전처리 로그를 보고 작성한다. 자동 scorer는 임의 모델의 학습 실행을 증명하지 않는다. TSCV 실행 증빙이 없으면 UNKNOWN이며 ELIGIBLE과 별도로 보고한다.
...
plan-forecast에서 개발 평가 뒤 승인된 model_spec의 canonical JSON SHA-256을 `final_selection.model_spec_hashes`에 넣는다. 코드의 `digest()`가 사용하는 규칙은 UTF-8, sort_keys=true, ensure_ascii=false, separators=(",",":")이다. 테스트를 보며 hash 목록을 다시 고르면 안 된다.
...
validation-forecast는 사후 탐색 감사로 표시하고 확인 가능한 코드·누락·결과만 보고한다. scorer 실행을 위해 사후 lock을 만들어 사전 동결인 것처럼 쓰지 않는다. 새 실험은 plan-forecast에서 별도 version으로 시작한다.
...
계획기의 fold_plan.csv는 학습에 허용된 원시 관측 창이다. 모델별 lag/label 변환 후 표본 수와 실제 refit 횟수는 실행 로그에서 확인한다. 임의 K-fold, nested 내부 split, gap/purge와 주기적 refit 실행은 별도 adapter가 담당한다.
