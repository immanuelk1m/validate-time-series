...
프로젝트의 설정·스키마·기존 실행기를 먼저 읽는다. 이미 확인 가능한 값을 다시 묻지 않는다. 목표 변수와 단위, 예측 시점과 시간대, horizon의 의미, 검증 구간, 기본 지표, 튜닝 예산을 확인한다. `h=4`가 관측치 네 개 뒤인지 달력상 4주 뒤인지 구분한다.

관측 사실과 논문 결과는 추정해 채우지 않는다. 파일 구조나 명백한 기본 경로처럼 결과를 거의 바꾸지 않는 가역적 선택은 합리적으로 정하고 가정으로 기록한다.
...
실험 전에 데이터 버전·forecast origins·horizons·전처리 학습 범위·재학습 주기, 주 지표·집계 가중치·seed·튜닝 예산·후보 목록·실패 처리·국면 정의를 고정한다. 개발 리더보드와 최종 holdout의 역할도 분리한다.
...
사용자가 명시적으로 protocol을 바꾸면 기존 결과를 덮어쓰지 말고 새 protocol/version으로 실행한다.
...
| 학습 label | 다중 horizon 학습 표본의 label 끝이 fit cutoff를 넘지 않는다. |
...
출력에 기록한 cutoff만으로 전처리 코드의 누수까지 입증할 수는 없다. 실제 코드 경로와 로그를 조사한 뒤 `audit.status=reviewed`와 근거를 남긴다. 미래 데이터만 바꾼 재실행에서 과거 origin의 입력·전처리 상태·예측이 변하는지도 확인한다.
...
기존 TFB·fev·자체 실행기가 있으면 재사용한다. 모델 내부 평가 코드를 그대로 신뢰하지 말고 원시 예측을 공통 형식으로 내보내 다시 채점한다. 각 adapter에는 해당 origin까지의 학습·입력 데이터와 알려진 미래 변수, horizon만 전달한다.
...
최소 키는 `series_id × origin × horizon × seed`다. target timestamp, 데이터/설정 hash, 모델/코드 버전, 학습·전처리 cutoff와 실패 사유도 저장한다.
...
실제 자료는 demo와 분리하고 원본 자료와 이전 결과를 덮어쓰지 않는다. 동봉 실행기는 chronological baseline과 예측 원장 평가를 지원한다. 외생변수 모델 학습, full HPO, 분산 실행, CRPS, conformal 학습, MCS/SPA/Clark-West/Giacomini-White는 기본 구현에 포함하지 않는다. 필요한 경우 검증된 외부 라이브러리를 adapter로 연결하고 그 경계를 명시한다.
