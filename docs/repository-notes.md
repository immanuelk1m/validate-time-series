...
입력은 `validate-time-series-v1.1.0-astra.zip`입니다. 기존 파일을 바꾸지 않고 경로만 `skills/` 아래로 옮겼습니다. 원본과 동일한지는 파일별 hash로 확인합니다. 논문 PDF는 배포하지 않으며 기존 `references/sources.md`를 유지합니다.
